...
def take_input(n):
    ...
    for client in clients:
        cars = client.recv(1024).decode()
        a,b = cars.split()
        #print(a,b)

        input_queue.append(int(a))
        input_queue.append(int(b))
...
def connect_to_clients():
    ...
    #to store the details of client sockets
    clients = []
    address = []
    ...
    for i in range(4):
        ...
        if i == 0:
            ...
            c = 'D'
        print('Connected with Client-%s...',c + addr[0] + "  " + str(addr[1]))

        #sending t (no of input to be taken)
