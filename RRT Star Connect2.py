...
def random_point():
    x = round(random.random() * l)
    y = round(random.random() * m)
    return (x,y)     #tupple
...
def steer(x_nearest, x_rand):
    ...
    if x_n != x:
        ...
        theta = math.pi
    #print(theta)
    a = round(x_n + Step * math.cos(theta))
    b = round(y_n + Step * math.sin(theta))
...
def connect(tree,vertex,x_new):
    ...
    '''
        if z>500:
            cv2.namedWindow("Connect?", cv2.WINDOW_NORMAL)
    ...
    for n in range(max_iteration):
        ...
        if msg != "TRAPPED":
            if connect(tree2,vertex2,x_new) == "REACHED":
                ...
                path = show_path(tree1,vertex1, tree1[vertex1.index(x_new)],start_node, img,0)
                path = show_path(tree2,vertex2, tree2[vertex2.index(x_new)],end_node, path,1)
                return path
        tree1, tree2 = swap(tree1,tree2)
        vertex1, vertex2 = swap(vertex1,vertex2)
        
    print("Max iteration reached...")
    #path = show_path(tree1, vertex1, tree1[vertex1.index(nearest(vertex1,end))],start_node, img)
    #return path

path = RRTStar_Connect(start, end)

cv2.namedWindow("ONCE MORE", cv2.WINDOW_NORMAL)
cv2.imshow("ONCE MORE", path)
cv2.waitKey(0)
