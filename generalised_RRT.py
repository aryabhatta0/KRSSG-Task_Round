...
#knowing starts & ends
a = int(input("Pick Start: (1/2/3): "))
b = int(input("Pick End: (1/2/3): "))
...
for i in range(l):
    for j in range(m):
        if switch1:
            if (img[i][j][0],img[i][j][1],img[i][j][2]) == (0,0,255):
                c+=1
                if c == a:
                    start = (i,j)
                    switch1 = False
                    ...
                    make_black((i,j))
        if switch2:
            if (img[i][j][0],img[i][j][1],img[i][j][2]) == (0,255,0):
                d+=1
                if d == b:
                    end = (i,j)
                    switch2 = False
...
#start = [(26,25),(76,27),(26,229)]
#end = [(287,95),(160,276),(282,271)]
...
def show_path(tree,vertex,current_node,final_node, img, flag= 0):
    ...
    while current_node != final_node:
        ...
        try:
            ...
            if t%30 ==0:                        #not selecting all index
                path.append((x1,y1))
        except:
            return img, path
